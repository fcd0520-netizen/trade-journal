  ...
  return (
    <div>
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="sm:col-span-2 lg:col-span-3"><label htmlFor="quick-watch-reason">監視理由</label><textarea id="quick-watch-reason" rows={3} value={form.reason} onChange={(e) => update("reason", e.target.value)} placeholder="監視を始めた理由や注目しているポイント" /></div>
        <div className="sm:col-span-2 lg:col-span-3"><button type="button" onClick={save} className="min-h-11 rounded-lg bg-sky-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-sky-500">追加する</button></div>
        {message && <p role="status" aria-live="polite" className={`rounded-xl border p-3 text-sm sm:col-span-2 lg:col-span-3 ${isError ? "border-rose-500/20 bg-rose-500/10 text-rose-300" : "border-emerald-500/20 bg-emerald-500/10 text-emerald-300"}`}>{message}</p>}
      </div>
    </div>
  );
}
